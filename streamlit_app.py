...
[Use LaTeX: $formula$ inline, $$formula$$ display]
...
def parse_json(text):
    ...
    2. Extracting from code fences
    3. Finding first and last braces
    """
    if not text:
        logger.debug("parse_json: empty input")
        return None
    ...
    if code_match:
        try:
            ...
            logger.debug("parse_json: code fence extraction failed")
            pass
    ...
    if first_brace != -1 and last_brace > first_brace:
        try:
            ...
            logger.debug("parse_json: brace extraction failed")
            pass

    logger.warning(f"parse_json: all strategies failed for input: {text[:100]}...")
    return None
...
def call_claude(system, user_msg, max_tokens=DEFAULT_MAX_TOKENS):
    """Single-turn Claude call with error handling and logging.
    ...
    """
    try:
        logger.info(f"Claude call: max_tokens={max_tokens}")
        ...
        logger.info(f"Claude response successful, tokens used: {r.usage.output_tokens}")
...
def call_claude_stream(system, messages, max_tokens=1500):
    """Streaming generator for multi-turn chat with error handling.
    ...
    """
    try:
        logger.info(f"Claude stream call: {len(messages)} messages")
        ...
        logger.error(f"Connection error in stream: {e}")
        yield "\n\n🌐 Connection error — check your internet."
    except anthropic.RateLimitError as e:
        logger.warning(f"Rate limited in stream: {e}")
        yield "\n\n⏳ Rate limited — wait a moment and try again."
    except anthropic.APIStatusError as e:
        logger.error(f"API error in stream {e.status_code}: {e.message}")
...
def check_and_log_feature(feature: str) -> Tuple[bool, str]:
    ...
    Args:
        feature: Feature name ("quiz", "past_paper", "plan", "learn")
        
    Returns:
        (can_access, message: warning or reason if blocked)
    """
...
def generate_quiz(topic, subject):
    ...
    Args:
        topic: Topic to generate question for
        subject: Subject context
        
    Returns:
        Validated MC question dict or None
    """
...
def generate_pp_mc(topic, subject):
    ...
    Returns:
        Validated past paper MC question dict or None
    """
    ...
    data = parse_json(text) if text else None
    if data and validate_mc(data):
        logger.info("Past paper MC generation successful")
        return data
...
def generate_pp_long(topic, subject):
    ...
    Returns:
        Structured question text or None
    """
...
def generate_marking(question_text, subject):
    ...
    Args:
        question_text: Question to generate marking scheme for
        subject: Subject context
        
    Returns:
        Marking scheme text or None
    """
...
def generate_plan(subjects_taken, exam_date, hours, weak, extra):
    ...
    Args:
        subjects_taken: Comma-separated subject names
        exam_date: Date of DSE exam
        hours: Daily study hours
        weak: Weakest areas description
        extra: Additional student info
        
    Returns:
        Study plan text or None
    """
    days_left = max((exam_date - datetime.now().date()).days, 1)
    logger.info(
        f"Generating study plan: {len(subjects_taken.split(','))} subjects, "
        f"{days_left} days, {hours}h/day"
    )
    prompt = (
        f"Student Profile:\n"
    ...
    plan = call_claude(PLANNER_SYSTEM.format(hours=hours), prompt, max_tokens=3500)
    if plan:
        logger.info("Study plan generation successful")
    else:
        logger.warning("Study plan generation failed")
    return plan
...
def reset_mode(mode_type="all"):
    ...
    Args:
        mode_type: "quiz", "pp_mc", "pp_long", or "all"
    """
    ...
    if mode_type in ("pp_mc", "all"):
        ...
        logger.info("Reset past paper MC state")
    ...
    if mode_type in ("pp_long", "all"):
        ...
        logger.info("Reset past paper long Q state")
...
def reset_quiz():
    """Reset quiz state (legacy)."""
    reset_mode("quiz")


def reset_pp_mc():
    """Reset past paper MC state (legacy)."""
    reset_mode("pp_mc")


def reset_pp_long():
    """Reset past paper long Q state (legacy)."""
    reset_mode("pp_long")
...
with st.sidebar:
    ...
    # Pricing section for non-premium users
    ...
    st.caption("Powered by Claude AI • v2.0 Premium")
    ...
    if st.session_state.plan is None:
        ...
        if submitted:
            ...
            if not subjects_taken:
                ...
                logger.warning("Plan generation: no subjects selected")
            elif not weak.strip():
                st.error(
                    "⚠️ Tell Athena your weak areas — "
                    "this is essential for a good plan!"
                )
                logger.warning("Plan generation: no weak areas specified")
            elif exam_date <= datetime.now().date():
                st.error("⚠️ Exam date must be in the future.")
                logger.warning(f"Plan generation: invalid exam date {exam_date}")
            elif hours < 1 or hours > 14:
                st.error("⚠️ Study hours must be between 1 and 14 per day.")
                logger.warning(f"Plan generation: invalid hours {hours}")
            else:
...
st.divider()
st.caption(
    "🏛️ **Athena AI** v2.0 — Built for DSE students, "
    "by a DSE student. &nbsp;|&nbsp; Powered by Claude AI"
)